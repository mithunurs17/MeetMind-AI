...
# ── Shared display helper (avoids duplicating the results block) ──────────
def _render_results(result: dict, meeting_title: str) -> None:
    ...
    if action_items:
        for item in action_items:
            st.markdown(
                f"<div class='glass-section'>"
                f"<b>{item.get('task')}</b><br/>"
    ...
    if risks:
        for risk in risks:
            ...
            st.markdown(
                f"<div class='glass-section' style='padding:12px 18px;'>• {text}</div>",
                unsafe_allow_html=True,
            )
    else:
    ...
    if questions:
        for question in questions:
            st.markdown(
                f"<div class='glass-section' style='padding:12px 18px;'>• {question}</div>",
                unsafe_allow_html=True,
            )
    else:
